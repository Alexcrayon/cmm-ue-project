...
struct CellCost
{
	...
	CellCost() : Cost(0.0f) {}
	CellCost(const FCellRef& Cell, float Cost) : Cell(Cell), Cost(Cost) {
...
class UGAPathComponent : public UActorComponent
{
	...
	bool Dijkstra(const FVector& StartPoint, FGAGridMap& DistanceMapOut, TMap<FCellRef, FCellRef>& PrevNodeOut) const;
	...
	bool BuildPathFromDistanceMap(const FVector& EndPoint, const FCellRef& EndCellRef, const FGAGridMap& DistanceMap, const TMap<FCellRef, FCellRef>& PrevNodeMap);

	float Heuristic(const FCellRef& Cell, const FCellRef& Dest) const;
	
	TArray<FCellRef> GetNeighbors(const FCellRef& Cell, const AGAGridActor* Grid) const;

	TArray<FCellRef> TracePath(const TMap<FCellRef, FCellRef>& CameFrom, FCellRef Start, FCellRef Dest) const;


	EGAPathState SmoothPath(const TArray<FPathStep>& UnsmoothedSteps, TArray<FPathStep>& SmoothedStepsOut) const;

	bool LineTrace(const FCellRef& Start, const FCellRef& End) const;
	void FollowPath();
