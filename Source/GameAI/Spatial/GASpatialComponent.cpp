	...
	: Super(ObjectInitializer)
{
	...
	//tried to set up last tick cell, not working
...
{
	UGAPerceptionComponent* Result = PerceptionComponentInternal.Get();
	if (Result)
	{
		return Result;
	}
	else
	{
		...
		if (Owner)
		{
			...
			if (Result)
			{
				PerceptionComponentInternal = Result;
			}
		}
		return Result;
	}
...
bool UGASpatialComponent::ChoosePosition(bool PathfindToPosition, bool Debug)
{
	...
	if (Grid->GridSpaceBoundsToRect2D(Box, CellRect))
	{
		...
		TMap<FCellRef, FCellRef> PrevNodeMap;
		
		FVector StartPoint = OwnerPawn->GetActorLocation();
		PathComponent->Dijkstra(StartPoint, DistanceMap, PrevNodeMap);
		...
		float LastVal;
		ScoreMap.GetValue(LastTickCell, LastVal);
		ScoreMap.SetValue(LastTickCell, LastVal + 20.0f);//add hysteresis, for now is arbitrary, the max y value in all curve must be 100.
		
		// (c) pick the best cell in ScoreMap
		float BestVal = -FLT_MAX;

		FCellRef BestCell;
		for (int32 i = 0; i <= ScoreMap.XCount; i++) {
			for (int32 j = 0; j <= ScoreMap.YCount; j++) {
				FCellRef CR;
				ScoreMap.LocalToCellRef(i, j, CR);

				float DistVal;
				DistanceMap.GetValue(CR, DistVal);
				if (DistVal == FLT_MAX) continue;

				float Value;
				ScoreMap.GetValue(CR, Value);
				if (Value > BestVal) {
					BestCell = CR;
					BestVal = Value;
				}
		...
		//successfully picked a best cell
		if (BestCell.IsValid()) {
			LastTickCell = BestCell;
			Result = true;
		}
		else {
			Result = false;
		...
		if (PathfindToPosition)
		{
			...
			if (Result) {
				FCellRef CurrCell = Grid->GetCellRef(StartPoint);
				if (CurrCell.Distance(LastTickCell) > 2) {

					FVector Loc = FVector((double)LastTickCell.X, (double)LastTickCell.Y, 0.0f);

					PathComponent->BuildPathFromDistanceMap(Loc, LastTickCell, DistanceMap, PrevNodeMap);
				}
				else // stop building the path if ai is already close to best cell
				{
					PathComponent->State = GAPS_Finished;
				}
			}
		}
...
void UGASpatialComponent::EvaluateLayer(const FFunctionLayer& Layer, const FGAGridMap& DistanceMap, FGAGridMap& ScoreMap) const
{
	...
	UGAPerceptionComponent* Perception = GetPerceptionComponent();
	...
	for (int32 Y = ScoreMap.GridBounds.MinY; Y < ScoreMap.GridBounds.MaxY; Y++)
	{
		for (int32 X = ScoreMap.GridBounds.MinX; X < ScoreMap.GridBounds.MaxX; X++)
		{
			...
			if (EnumHasAllFlags(Grid->GetCellData(CellRef), ECellData::CellDataTraversable))
			{
				...
				float Value = 0.0f;
				if (Layer.Input == SI_None) {
					continue;
				}
				else if (Layer.Input == SI_TargetRange) {
					
					FTargetState StateOut;
					FTargetView ViewOut;
					if (Perception->GetCurrentTargetState(StateOut, ViewOut)) {
						FVector PlayerLoc = StateOut.Position;
						//FVector PlayerLoc = PlayerPawn->GetActorLocation();
						FCellRef PlayerCell;
						PlayerCell = Grid->GetCellRef(PlayerLoc);
						Value = CellRef.Distance(PlayerCell);
				...
				else if (Layer.Input == SI_PathDistance) {
					DistanceMap.GetValue(CellRef, Value);
					if (Value == FLT_MAX) continue; //ignore unreachable cells
				}
				else if (Layer.Input == SI_LOS) {
					FTargetState StateOut;
					FTargetView ViewOut;
					FVector PlayerLoc;
					if (Perception->GetCurrentTargetState(StateOut, ViewOut)) {
						PlayerLoc = StateOut.Position;
					...
					FVector End = PlayerLoc;		// need a ray end
				...
				float OldVal;
				ScoreMap.GetValue(CellRef, OldVal);
				if (Layer.Op == SO_None) {
					continue;
				}
				else if (Layer.Op == SO_Add) {
					CombinedValue = OldVal + ModifiedValue;
				}
				else if (Layer.Op == SO_Multiply) {
					CombinedValue = OldVal * ModifiedValue;					
				}
